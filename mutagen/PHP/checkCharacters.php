...
if (!$stmt->fetch()) echo  json_encode(array('status'=>'You are not logged in.'));
	...
	if ($key == $row_sessionkey)  { //  if the user owns the current active session
		...
		$stmt = $conn->prepare("SELECT id, name, class, level FROM characters WHERE user_id = ? ");
		
		$stmt->bind_param("s", $userid);
		
		$stmt->execute();
				
		$stmt->bind_result($row_id, $row_name, $row_class, $row_level );
					
		while ($stmt->fetch()) 
		{
			//add to array of characters:	
			$chararray[] = array('id' => $row_id, 'name'=>$row_name, 'class'=>$row_class, 'level'=>$row_level);
		}
